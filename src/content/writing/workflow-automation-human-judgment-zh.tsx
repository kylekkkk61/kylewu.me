    ...
    <div className="space-y-14 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:tracking-tight [&_h3]:text-lg [&_h3]:font-semibold [&_p]:text-lg [&_p]:leading-8 [&_p]:text-muted-foreground">
      <section className="space-y-5">
        <p>
          ...
          能完成哪些任務，或一套流程能省下多少時間。但在選擇工具以前，更應該先理解工作本身。
        </p>
        <p>
        ...
        <p>
          AI 可以加速其中部分工作，但 AI
          本身不是完整的工作流程。真正有用的流程，通常從標準化開始，而不是從自動化開始。
        </p>
      </section>
      ...
      <section className="space-y-5" aria-labelledby="standardize-first-zh">
        ...
        <p>
          報名資料包含個人經歷、參加動機、對活動的期待、學習興趣、課外活動，以及對未來升學或職涯方向的想法。這些資訊無法只靠單一數字判斷，但如果完全依賴每位評分者的第一印象，也很難維持一致。
        </p>
        <p>
          因此，我們建立了一套小型審查流程。評分團隊約有五人，每位報名者平均會由三人交叉評分。正式評分以前，我們先說明不同情況應如何給分、需要觀察哪些面向，以及為什麼評分時應保持中立，避免只因為某位報名者的表達方式比較熟悉，就給予額外優勢。
        </p>
        <p>
          評分面向包含對企管系的興趣、對企管領域的基本認識、報名動機、高中階段的學業與個人經歷、對未來方向的思考，以及社團、專案、競賽等課外參與。原始的權重文件目前已經找不到，因此我不會事後重建一套看似精確的公式。這套流程真正的價值，也不在於產生一個「完美分數」，而是讓所有評分者先使用相近的結構理解資料。
        </p>
        <p>
        ...
        <p>
          這套流程完全不需要
          AI，但也沒有試圖消除人工判斷。評分標準降低了隨意性，也讓意見不一致時有跡可循；最後仍要有人重新閱讀資料、回到營隊的招募目的，判斷哪些差異真正重要。
        </p>
        <p>評分表可以為決策準備資訊，但不應假裝自己就是決策。</p>
      </section>

      <section className="space-y-5" aria-labelledby="structured-iteration-zh">
        <h2 id="structured-iteration-zh">
          數據可以讓迭代更有結構，但不會替產品說話
        </h2>
        <p>
          後來，我在協助 CAVE
          烤爐群眾募資專案的數位行銷與產品溝通時，遇到另一種工作流程。
        </p>
        <p>
          募資首頁、廣告圖片、標語、文案、資訊順序與投放受眾之間彼此影響，每一項看似只是細節，卻都可能改變使用者如何理解產品。如果所有調整都只依賴個人偏好，流程容易失去一致性；但如果把後台數字直接當成答案，也同樣過度簡化。
        </p>
        <p>
        ...
        <p>
          數據可以協助定位問題，也能縮小接下來需要回答的問題；但它不會自動決定產品應該如何被理解，也不會直接告訴團隊哪一句話最能代表產品。這仍然需要對產品、受眾與溝通方式的理解，也需要知道同一時間還有哪些變因正在改變。
        </p>
        <p>
          這同樣是一條決策邊界。監測、彙整與提醒可以自動化；數字為什麼出現，以及答案是否真的應該是修改文案，仍可能需要人判斷。
        </p>
      </section>

      <section className="space-y-5" aria-labelledby="decision-boundary-zh">
        <h2 id="decision-boundary-zh">讓系統把工作推進到決策邊界</h2>
        <p>企管營的預算規劃，則讓同一項原則變得更具後果。</p>
        <p>
          營隊預計招收 120
        ...
        <p>
          我們曾使用當時可用的生成式 AI
          工具，協助提出預算規劃、可能的支出調整與影響。它可以提供另一個觀察問題的角度，但除非我們完整說明每項限制、非正式依賴與活動品質要求，否則工具不可能自行擁有完整脈絡。
        </p>
        <p>
          因此，團隊仍自行建立了十多個預算版本，涵蓋不同招收人數與營運情境。AI
          可以協助產生方案，最後仍需要人檢查假設，並在成本、安全、場地容量、交通、食材與參與者體驗之間做出選擇。
        </p>
        <p>
          當兩個場地在價格、容量與便利性上都很接近時，評分表也許只能告訴我們兩者差不多。它無法替團隊承擔最後的選擇。仍然需要有人看見模型沒有記錄的情境差異，判斷哪一種活動體驗比較重要，並接受這項選擇帶來的結果。
        </p>
        <p>
          一套好的流程，應該把決策以前能做的工作盡量做好：整理資料、計算不同情境、找出矛盾、比較假設，並準備需要人工確認的問題。但不能只因為所有數字都能放進試算表，就把具有後果的選擇包裝成完全客觀的輸出。
        </p>
      </section>
      ...
      >
        ...
        <p>
          判斷一件事值不值得自動化時，發生頻率很重要；但判斷自動化應該停在哪裡時，後果通常更重要。
        </p>
        <p>
          一項經常發生、影響有限、輸入穩定的重複工作，很適合自動化。涉及金流、安全或不可逆產品行動時，設計方式就必須不同。系統仍然可以完成大部分準備工作，但必須把假設呈現出來，並保留清楚的人工確認點。
        </p>
        <p>
          後來，我也把這項原則直接運用在 Kaiyn Trading
          Bot。系統可以驗證交易所規則、計算部位、保存待處理狀態並準備訂單，但不會把社群訊號直接視為下單許可。訊號發布前先由發送者確認，最後計算完成的訂單則由使用者再次確認。
        </p>
        <p>
          如果交易所回應不確定，系統會先查詢並比對既有訂單狀態，而不是把不確定直接解讀成重新送單的許可。在這個情境裡，自動重試或許比較方便，卻也可能帶來實際的金流風險。
        </p>
        <p>
          這不代表重要操作都應該增加更多警告或點擊。人工確認只有在提供了能改變理解的資訊時才有價值，例如計算假設、衍生數值、目前限制，以及繼續操作可能造成的結果。
        </p>
      </section>
      ...
      <section className="space-y-6" aria-labelledby="decision-framework-zh">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            <h3>5. 系統犯錯時會發生什麼？</h3>
            <p>
              需要考慮結果能否復原、金流影響、安全、隱私，以及修正錯誤的成本。後果越高，就越需要嚴格驗證、稽核紀錄與人工同意。
            </p>
          </div>
          <div className="space-y-3">
      ...
      <section className="space-y-5" aria-labelledby="judgment-not-step-zh">
        ...
        <p>
          AI
          可以在隱私與權限條件明確時協助整理資料、摘要回饋、計算情境、產生方案，或找出異常數據。這些能力能讓標準化流程更快，也更一致，但不會替團隊決定整套系統到底要優先追求什麼。
        </p>
        <p>
          有些選擇沒有唯一的客觀答案。較便宜的場地與較舒適的場地，可能分別對應不同但合理的優先順序；兩位報名者分數相近，卻可能帶著不同類型的好奇心；某一則文案可能提高點擊，卻讓產品被理解成另一件事。
        </p>
        <p>
      ...
      <aside className="border-border bg-muted/30 space-y-4 rounded-xl border p-6 md:p-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-base! leading-7!">
            ...
            AI
            改善分析與準備，同時讓真正具有後果的判斷保持可見、可覆核，並由人明確承擔。
          </p>
        </div>
      </aside>
    </div>
